...
RECORDING_DIR = Path("recordings/gridworld_ql")
    ...
    def run_training(
        ...
        for episode in tqdm(range(num_episodes), desc="Training"):
            ...
            if should_record:
                ...
                # Save Q-table snapshot for the recorded episode.
                q_snapshot = {
                    state: values.copy() for state, values in self.agent.q_table.items()
                }
...
def main():
    ...
    visualizer.record_final(
        output_path=str(RECORDING_DIR / "final.png"),
        trajectory_path=str(RECORDING_DIR / "trajectory.png"),
    )
    ...
    if rewards_path is not None:
        plot_rewards(
            rewards_path,
            smooth_window=50,
            output_path=str(RECORDING_DIR / "rewards.png"),
        )
